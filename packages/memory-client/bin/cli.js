...
const { setSlot, getSlot, deleteSlot, listSlots, getSlotCount, FREE_SLOTS } = require("../src/slots");
...
function getAddresses() {
  const protocolAddr = process.env.PROTOCOL_ADDRESS || "0x3BD7945d18FE6B68D273109902616BF17eb40F44";
  const mmpAddr      = process.env.MMP_TOKEN_ADDRESS || "0x30b8Bf35679E024331C813Be4bDfDB784E8E9a1E";
...
program
  ...
  .option("-s, --slot <name>", "Save to named slot (max 10, LRU eviction)")
  .action(async (filePath, opts) => {
    try {
      ...
      const os = require("os");
      const metaDir = path.join(os.homedir(), ".clawmemory", "roots");
      ...
      const cacheDir = path.join(os.homedir(), ".clawmemory", "cache", root.replace("0x", "").slice(0, 16));
      fs.mkdirSync(cacheDir, { recursive: true });
      fs.writeFileSync(path.join(cacheDir, "combined.bin"), combined);

      // ── Named slot ────────────────────────────────────────────────────────
      if (opts.slot) {
        const { evicted } = setSlot(wallet.address, opts.slot, root, timestamp);
        const count = getSlotCount(wallet.address);
        console.log(`Slot [${opts.slot}] saved (${count}/${FREE_SLOTS} slots used)`);
        if (evicted) console.log(`  ↳ Evicted oldest slot: [${evicted}]`);
      }
...
program
  .command("load <merkleRootOrSlot> <outFile>")
  .description("Download and decrypt a memory tree (pass merkleRoot or --slot <name>)")
  .option("-s, --slot <name>", "Load by named slot instead of merkleRoot")
  .action(async (merkleRootOrSlot, outFile, opts) => {
    // resolve slot → root
    if (opts.slot) {
      const wallet = await loadWallet();
      const entry  = getSlot(wallet.address, opts.slot);
      if (!entry) {
        console.error(`Slot [${opts.slot}] not found. Use 'clawmemory slot list' to view slots.`);
        process.exit(1);
      }
      merkleRootOrSlot = entry.root;
      if (!process.env.OMP_KEY_TIMESTAMP && !process.argv.find(a => a.startsWith("--timestamp="))) {
        process.env.OMP_KEY_TIMESTAMP = String(entry.timestamp);
      }
      console.log(`Slot [${opts.slot}] → ${merkleRootOrSlot}`);
    }
    const merkleRoot = merkleRootOrSlot;
    return _loadImpl(merkleRoot, outFile);
  });

async function _loadImpl(merkleRoot, outFile) {
  try {
      ...
      const storageDir = path.join(
        ...
        ".clawmemory", "cache", merkleRoot.slice(0, 16)
      );
  ...
  .action(async (merkleRoot, opts) => {
    try {
      ...
      const autoCacheFile = path.join(
        os.homedir(), ".clawmemory", "cache", merkleRoot.replace("0x", "").slice(0, 16), "combined.bin"
      );
...
const slotCmd = program
  .command("slot")
  .description("Manage named memory slots (local index, max 10 per address)");

slotCmd
  .command("list")
  .description("List all named slots")
  .action(async () => {
    try {
      const wallet = await loadWallet();
      const slots  = listSlots(wallet.address);
      if (slots.length === 0) {
        console.log("No named slots yet. Use: clawmemory save <file> --slot <name>");
        return;
      }
      console.log(`\n Named Slots — ${wallet.address} (${slots.length}/${FREE_SLOTS})`);
      console.log("─".repeat(72));
      for (const s of slots) {
        const ago = Math.round((Date.now() - s.savedAt) / 60000);
        console.log(` [${s.name.padEnd(24)}] ${s.root.slice(0, 20)}...  ${ago}m ago`);
      }
      console.log("─".repeat(72));
    } catch (err) {
      console.error("slot list error:", err.message);
      process.exit(1);
    }
  });
...
slotCmd
  .command("get <name>")
  .description("Print merkleRoot and timestamp for a named slot")
  .action(async (name) => {
    try {
      const wallet = await loadWallet();
      const entry  = getSlot(wallet.address, name);
      if (!entry) {
        console.error(`Slot [${name}] not found.`);
        process.exit(1);
      }
      console.log(`Slot:      ${name}`);
      console.log(`Root:      ${entry.root}`);
      console.log(`Timestamp: ${entry.timestamp}`);
      console.log(`Saved:     ${new Date(entry.savedAt).toISOString()}`);
      console.log(`\nLoad: clawmemory load ${entry.root} <outFile> --timestamp=${entry.timestamp}`);
      console.log(`  Or: clawmemory load --slot ${name} <outFile>`);
    } catch (err) {
      console.error("slot get error:", err.message);
      process.exit(1);
    }
  });
...
slotCmd
  .command("delete <name>")
  .description("Remove a named slot (local only, does not affect on-chain data)")
  .action(async (name) => {
    try {
      const wallet  = await loadWallet();
      const removed = deleteSlot(wallet.address, name);
      if (removed) {
        console.log(`Slot [${name}] removed. On-chain data is unaffected.`);
      } else {
        console.error(`Slot [${name}] not found.`);
        process.exit(1);
      }
    } catch (err) {
      console.error("slot delete error:", err.message);
      process.exit(1);
    }
  });
