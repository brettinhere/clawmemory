...
async function main() {
  ...
  console.log("═══════════════════════════════════════════════════");
  console.log("  ClawMemory Miner v1.0.1");
...
function spawnWorker(workerIndex, wallet, chain, hcServer) {
  ...
  worker.on("message", async (msg) => {
    ...
    try {
      ...
      await submitSolution(nonce, currentChallenge, wallet, chain, hcServer);
    } catch (err) {
...
async function submitSolution(nonce, challenge, wallet, chain, hcServer) {
  const { targetRoot, epochSeed } = challenge;
  ...
  // Contract: uint256(epochSeed) % tree.totalChunks
